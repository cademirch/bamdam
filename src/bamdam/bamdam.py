...
def write_shortened_lca(
    original_lca_path, short_lca_path, upto, mincount, exclude_keywords, lca_file_type
):
    ...
    with open(original_lca_path, "r") as infile, open(short_lca_path, "w") as outfile:
        ...
        for line in infile:
            ...
            if newreadname == oldreadname:
                print(
                    "Error: You have duplicate entries in your LCA file, for example "
                    + newreadname
                    + ". You should fix this and re-run bamdam. Here is a suggested fix: awk '!seen[$0]++' input_lca > deduplicated_lca"
                )
                exit(-1)
            if upto in line:
                entry = line.strip().split("\t")
                if len(entry) > 1:
                    ...
                    for field in fields:
                        number = (
                            field.split(":")[0].strip("'").strip('"')
                        )  # metadmg lca files can have quotation marks everywhere
                        level = field.split(":")[2].strip("'").strip('"')
                        if level == upto:
                            if (
                                number in goodnodes
                            ):  # you only need to check the upto counts, as they will be higher than anything underneath them
                                if lca_file_type == "ngslca":
                                    outfile.write(line)
                                elif lca_file_type == "metadmg":
                                    ...
                                    fullentry = (
                                        "\t".join([firstentry, restentry]).replace(
                                            '"', ""
                                        )
                                        + "\n"
                                    )
...
def write_shortened_bam(
    ...
    with pysam.AlignmentFile(
        ...
        progress_bar = (
            tqdm(
                total=totallcalines - lcaheaderlines,
                unit="lines",
                disable=not tqdm_imported,
            )
            if tqdm_imported
            else None
        )
        if progress_bar:
            update_interval = (
                100  # this is arbitrary. could also be e.g. (totallcalines // 1000)
            )
        ...
        while notdone:
            if bamread.query_name == lcareadname:
                ...
                readlength = (
                    bamread.query_length
                )  # same read length for all the alignments
                similarity = (
                    1 - bamread.get_tag("NM") / readlength
                )  # not the same NM for all the alignments
                        ...
                        if bamread.query_name == lcareadname:
                            ...
                            if similarity >= minsimilarity:
                                if annotate_pmd:
                                    ...
                                    bamread.set_tag(
                                        "DS", "%.3f" % pmd
                                    )  # replace a tag if it's already there
                                outfile.write(bamread)  # write the read!
                        else:
                            currentlymatching = False
                    except StopIteration:
...
def get_mismatches(seq, cigar, md):
    ...
    for x in cig:
        ...
        if (
            cat == "S"
        ):  # soft clip: bases are present in the read, but not the reference. soft clipping is enabled by default in bwa but not bowtie2.
            ...
            if x == cig[len(cig) - 1]:  # the end of the read is soft clipped
                ...
                sys.exit(
                    "Error: It looks like one of your cigar strings "
                    + cigar
                    + " encodes a soft clip internally to a read. That shouldn't be happening."
                )

            # now pad the sequences as needed
            read_seq += (
                "s" * bases
            )  # soft clipped. i don't care what you are. # seq[read_pos:read_pos + bases]
            ...
            cat in ["D", "N"]
        ):  # 'D' : the reference has something there and the read doesn't, but pad them both
            ref_seq += (
                "N" * bases
            )  # N means it's an ACTG, we just don't know which one right now
            read_seq += "-" * bases

        elif (
            cat == "I"
        ):  # I: the read has something there and the reference doesn't, but pad them both
    ...
    for x in md_list:
        if x.startswith("softclip"):
            ...
            "^"
        ):  # this can be arbitrarily long (a ^ and then characters)
            ...
            if x.isdigit():  # can be multiple digits
                ...
                read_pos = len(read_up_to_here) - read_up_to_here.count(
                    "-"
                )  # we padded it with "-"s earlier
                ...
                if refhere is None or readhere is None:
                    print(
                        "Warning: There appears to be an inconsistency with seq "
                        + seq
                        + " and cigar "
                        + cigar
                        + " and md "
...
def mismatch_table(read, cigar, md, flagsum):
    ...
    matchs = []
    for i, (read_char, ref_char) in enumerate(
        zip(readseq_no_read_dashes, refseq_no_read_dashes)
    ):
        if (
            read_char == ref_char and read_char in "ACTG"
        ):  # only count matches if both are an A C T or G
...
def get_rep_kmer(
    seq,
):  # representative canonical kmer representation for counting, gets the lexicographical min of a kmer and its rev complement
...
def get_pmd(read, stranded):
    ...
    bit_position = 4  #  2^4 = 16 this flag means it's aligned in reverse
    backwards = (
        len(bin_flagsum) > bit_position and bin_flagsum[bit_position] == "1"
    )  # backwards is a boolean
    ...
    if stranded == "ss":
        for b in range(0, readlength):
            ...
            if refseqlist[b] == "C" and (
                readseqlist[b] == "T" or readseqlist[b] == "C"
            ):
    ...
    if stranded == "ds":
        for b in range(0, readlength):
            ...
            if refseqlist[b] == "G" and (
                readseqlist[b] == "A" or readseqlist[b] == "G"
            ):
                # get distance and stuff to 3 prime end
                epsilon = (
                    1 / 3 * 10 ** (-phred[b] / 10)
                )  # phred score 30 gives an error rate of 0.001 (then * 1/3)
...
def gather_subs_and_kmers(bamfile_path, lcafile_path, kn, upto, stranded):
    ...
    if tqdm_imported:
        ...
        if progress_bar:
            update_interval = (
                100  # this is arbitrary ofc. could also be e.g. (totallcalines // 1000)
            )
    ...
    for read in bamfile:
        ...
        if readname != oldreadname and oldreadname != "":
            ...
            for node in nodestodumpinto:
                ...
                if node not in node_data:
                    if are_pmds_in_the_bam:
                        node_data[node] = {
                            ...
                            "tax_path": "",
                            "subs": {},
                            "damagerelevantreadsp1": 0,
                            "damagerelevantreadsm1": 0,
                            "dp1": 0,
                            "dm1": 0,
                ...
                node_data[node]["meanlength"] = (
                    (node_data[node]["meanlength"] * node_data[node]["total_reads"])
                    + readlength
                ) / (node_data[node]["total_reads"] + 1)

                if dust != -1:
                    node_data[node]["avgdust"] = (
                        (node_data[node]["avgdust"] * node_data[node]["total_reads"])
                        + dust
                    ) / (node_data[node]["total_reads"] + 1)
                else:
                    readswithNs += 1
                ...
                node_data[node]["ani"] = (
                    ani_for_this_read
                    + node_data[node]["ani"] * node_data[node]["total_reads"]
                ) / (node_data[node]["total_reads"] + 1)
                gc_content_for_this_read = (
                    seq.count("C") + seq.count("G")
                ) / readlength
                node_data[node]["avgreadgc"] = (
                    (node_data[node]["avgreadgc"] * node_data[node]["total_reads"])
                    + gc_content_for_this_read
                ) / (node_data[node]["total_reads"] + 1)
                ...
                if currentsubdict:
                    for sub, count in currentsubdict.items():
                        ...
                        if sub in node_data[node]["subs"]:
                            ...
                            node_data[node]["subs"][sub] = (
                                count / num_alignments
                            )  # so, this can be up to 1 per node.
                # add the tax path if it's not already there
                if node_data[node]["tax_path"] == "":
                    try:
                        lca_index = next(
                            i
                            for i, entry in enumerate(lcaentry)
                            if entry.split(":")[0].strip("'").strip('"') == node
                        )
        ...
        if (
            (readname != oldreadname)
            or (cigar != oldcigar)
            or (md != oldmd)
            or (flagsum != oldflagsum)
        ):
    ...
    if lcalinesskipped > 0:
        print(
            "\nWarning: "
            + str(lcalinesskipped)
            + " reads in the input LCA file did not appear in the input bam file and so were not used. This may have happened if the minimum similarity used in bamdam shrink did not match that used in ngsLCA. This will not affect output statistics, except that these reads will not be included."
        )

    if readswithNs > 0:
        print(
            "\nSkipped k-mer counting and DUST score computation for "
            + str(readswithNs)
            + " reads with non-ACGT characters. \n"
    ...
    print(
        "\nGathered substitution and kmer data for "
        + str(len(node_data))
        + " taxonomic nodes. Now sorting and writing output files... "
    )
...
def format_subs(subs, nreads):
    ...
    for key, value in subs.items():
        ...
        if (
            (-15 <= pos <= 15)
            and (parts[0] in {"A", "C", "T", "G"})
            and (parts[1] in {"A", "C", "T", "G"})
        ):
...
def calculate_node_damage(subs, stranded):
    ...
    dm1 = (
        (ctm1 / c_m1 if c_m1 > 0 else 0)
        if stranded == "ss"
        else (gam1 / g_m1 if g_m1 > 0 else 0)
    )
    ...
    for node in nodedata:
        ...
        if pmds_in_bam:
            row = [
                ...
                round(dp1, 4),
                round(dm1, 4),
                round(tn["meanlength"], 2),
                round(tn["ani"], 4),
                round(tn["avgreadgc"], 3),
                round(avgrefgc, 3),
                numuniquekmers,
                round(ratiodup, 3),
                tn["total_alignments"],
                round(tn["pmdsover2"] / tn["total_reads"], 3),
                round(tn["pmdsover4"] / tn["total_reads"], 3),
                tn["tax_path"],
            ]
        else:
            row = [
...
def extract_reads(
    in_lca, in_bam, out_bam, tax, subset_header=False, only_top_ref=False
):
    ...
    if only_top_ref and not subset_header:
        print(
            "Error: The --only_top_ref flag requires the --subset_header flag to be set. Please set both flags and re-run."
        )
        sys.exit()
    ...
    if not subset_header and not only_top_ref:
        ...
        hashtax = abs(
            hash(tax_pattern)
        )  # ensures no accidental file overwriting; the abs is because sometimes the hash is negative
    ...
    if only_top_ref:
        ...
        header["SQ"] = [
            sq for sq in header.get("SQ", []) if sq["SN"] in reference_names
        ]
            ...
            for read in bam_reader_again:
                if read.query_name in read_names:
                    ...
                    if read.reference_id >= 0:
                        ref_name = bam_reader_again.get_reference_name(
                            read.reference_id
                        )
...
def calculate_damage_for_plot(items):
    ...
    }  # other mismatches at 5' positions (not including C>T or G>A)
    other_3prime = {
        i: 0 for i in range(1, 16)
    }  # other mismatches at 3' positions (not including C>T or G>A)
    total_nondeam_5prime = {
        i: 0 for i in range(1, 16)
    }  #  denominator part : all non C>T or G>A matches or mismatches at each position
    ...
    for item in items:
        ...
        if 1 <= pos <= 15:
            ...
            if from_base != to_base:
                if (from_base == "C" and to_base == "T") or (
                    from_base == "G" and to_base == "A"
                ):
                    continue
                else:
    ...
    proportion_ct_5prime = {
        i: ctp_5prime[i] / total_c_5prime[i] if total_c_5prime[i] > 0 else 0
        for i in range(1, 16)
    }
    proportion_ga_5prime = {
        i: gap_5prime[i] / total_g_5prime[i] if total_g_5prime[i] > 0 else 0
        for i in range(1, 16)
    }
    proportion_ct_3prime = {
        i: ctp_3prime[i] / total_c_3prime[i] if total_c_3prime[i] > 0 else 0
        for i in range(1, 16)
    }
    proportion_ga_3prime = {
        i: gap_3prime[i] / total_g_3prime[i] if total_g_3prime[i] > 0 else 0
        for i in range(1, 16)
    }
    proportion_other_5prime = {
        i: other_5prime[i] / total_nondeam_5prime[i]
        if total_nondeam_5prime[i] > 0
        else 0
        for i in range(1, 16)
    }
    proportion_other_3prime = {
        i: other_3prime[i] / total_nondeam_3prime[i]
        if total_nondeam_3prime[i] > 0
        else 0
        for i in range(1, 16)
    }
...
def make_damage_plot(in_subs_list, in_subs, tax, plotfile, ymax=0):
    ...
    for file in subs_files:
        ...
        if len(matched_line) == 0:
            print(
                f"Warning: {file} does not contain an entry for {tax}. Skipping this file."
            )
            continue
        elif len(matched_line) > 1:
            raise ValueError(
                f"More than one matching line found in {file}. Please be more specific, e.g., by using a tax ID instead of a name."
            )
        ...
        ctp, gap_5prime, ctm, gap_3prime, other_5prime, other_3prime = (
            calculate_damage_for_plot(data_items)
        )
    ...
    if ymax == 0 or ymax == "0":
        max_y = min(
            1.0,
            max(
                max(item for sublist in values_all_files[key] for item in sublist)
    ...
    labels1 = [
        ...
        "C to T",
        "G to A",
    ]  # labels for legend, one entry for each damage type
    ...
    ax2.legend(
        handles=handles2, labels=labels1, loc="upper left", fontsize=12
    )  # legend
    ...
    if file_extension == ".pdf":
        ...
        if file_extension != ".png":
            print(
                "Warning: Invalid plot file suffix. Your plot file is being saved in png format with the filename you requested."
            )
...
def make_baminfo_plot(in_bam, in_bam_list, plotfile):
    ...
    for i, bam_file in enumerate(bamfiles):
        ...
        ax1.plot(
            ...
            y_vals,
            linestyle="-",
            label=os.path.basename(bam_file),
    ...
    if file_extension == ".pdf":
        ...
        if file_extension != ".png":
            print(
                "Warning: Invalid plot file suffix. Your plot file is being saved in .png format, using the filename you specified."
            )
        plt.savefig(plotfile)
...
def parse_exclude_keywords(args):
    if hasattr(args, "exclude_keywords") or hasattr(args, "exclude_keyword_file"):
        if args.exclude_keywords and args.exclude_keyword_file:
            raise ValueError(
                "Please only provide one of --exclude_keywords or --exclude_keyword_file, not both."
            )
...
def tsvs_to_matrix(parsed_data, output_file, include="all", minreads=50):
    ...
    for (
        sample_name,
        records,
    ) in parsed_data.items():  # parsed_data is a dict: {sample_name: records}
    ...
    sorted_tax_data = sorted(
        tax_data.items(), key=lambda x: x[1]["TotalReads"], reverse=True
    )
    ...
    with open(output_file, "w") as outfile:
        ...
        for tax, data in sorted_tax_data:
            ...
            for sample_name in sorted(parsed_data.keys()):
                ...
                if include_damage:
                    row.append(
                        str(sample_data.get("damage"))
                        if sample_data.get("damage") is not None
                        else "NA"
                    )
                if include_duplicity:
                    row.append(
                        str(sample_data.get("duplicity"))
                        if sample_data.get("duplicity") is not None
                        else "NA"
                    )
                if include_dust:
                    row.append(
...
def make_krona_xml(in_tsv, in_tsv_files, out_xml, minreads, maxdamage):
    ...
    for file in input_files:
        ...
        with open(file, "r") as f:
            ...
            if header[0] != "TaxNodeID":
                print(
                    "Error: It looks like your input tsv file was not generated from bamdam compute or bamdam combine, or perhaps you edited the header."
                )
                continue
            ...
            try:
                ...
                if reads < minreads:
                    print(
                        f"Warning: File {file} only contains taxa with at most ({reads}), which is below minreads ({minreads}). This file will be excluded."
                    )
                    continue
            except (ValueError, IndexError):
                print(
                    f"Error: File {file} has an unexpected format and will be excluded."
                )
                continue
    ...
    for file in input_files:
        ...
        )  # tsvs are ordered; top line is max reads
        ...
        for line in lines[1:]:  # skip the header
            ...
            dup = float(fields[3])
            dust = float(fields[4])
            damage = float(fields[5])
            lengths = float(fields[7])
            ...
            if reads >= minreads:
                if (
                    taxid not in tree
                ):  # add a new node to the tree structure if it's not already there
                    tree[taxid] = {
                        "taxname": taxname,
                        "samples": {
                            sample_name: {
                    ...
                    if sample_name not in tree[taxid]["samples"]:
                        tree[taxid]["samples"][sample_name] = {
                            "reads": reads,
                            "duplicity": dup,
                            "dust": dust,
                            "damage": damage,
                ...
                if level != toplevel:
                    ...
                    parent_taxname = parent[1]
                    parent_level = parent[2]
                    if parent_taxid not in tree:
                        # initialize it. we'll fill it in later. it must be there.
                        tree[parent_taxid] = {
                            "taxname": parent_taxname,
                            "samples": {},
                            "taxpath": ", ".join(taxpathsplit[1:]),
                            "children": set(),
                        }
        ...
        root_reads = sum(
            tree[taxid]["samples"][sample_name]["reads"]
            for taxid in tree
            if sample_name in tree[taxid]["samples"]
            and not any(taxid in tree[child_id]["children"] for child_id in tree)
        )
        ...
        sample_max_damage[sample_name] = max(
            ...
            if sample_name in node["samples"]
            and node["samples"][sample_name]["damage"] is not None
        )
    ...
    if len(input_files) > 1:
        ...
        for taxid, node in tree_items:
            ...
            avg_duplicity = (
                sum(duplicity_values) / len(duplicity_values) if duplicity_values else 0
            )
            avg_dust = sum(dust_values) / len(dust_values) if dust_values else 0
            damage = (
                weighted_damage_sum / total_reads_for_damage
                if total_reads_for_damage > 0
                else 0
            )
    ...
    if maxdamage is not None:
        ...
        overall_max_damage = max(
            overall_max_damage, 0.3
        )  # otherwise auto-detect but ensure at least 0.3
    ...
    def add_node_to_xml(node_id, tree, indent_level=2):
        ...
        node_str += (
            f"{indent}\t<taxid><val>{node_id}</val></taxid>\n"  # add the tax id too
        )
    ...
    with open(out_xml, "w") as file:
        ...
        file.write(
            f'\t<color attribute="reads" valueStart="0" valueEnd="{overall_max_reads}" hueStart="120" hueEnd="240"></color>\n'
        )
        file.write(
            f'\t<color attribute="damage" valueStart="0" valueEnd="{overall_max_damage}" hueStart="0" hueEnd="240"></color>\n'
        )
        file.write("\t<datasets>\n")
        for sample_name in sample_names:
        ...
        for root_taxid in [
            taxid
            for taxid in tree
            if not any(taxid in tree[child_id]["children"] for child_id in tree)
        ]:
            try:
                file.write(add_node_to_xml(root_taxid, tree))
            except RecursionError:
                print(
                    f"There was a recursion error for {root_taxid}. We will skip it. If your tree is truly ginormous, you can increase the default recursion limit."
                )
                continue
...
def shrink(args):
    ...
    if lca_file_type == "metadmg":
        print(
            "You are running bamdam shrink with a metaDMG-style lca file. This is ok, but be aware the output lca file will be in ngsLCA lca file format, as all the other functions in bamdam require this format."
        )
    shortlcalines = write_shortened_lca(
        args.in_lca,
        args.out_lca,
        args.upto,
        args.mincount,
        formatted_exclude_keywords,
        lca_file_type,
    )
    write_shortened_bam(
        args.in_bam,
...
def compute(args):
    ...
    if lca_file_type == "metadmg":
        print(
            "Error: It looks like you're trying to run bamdam compute with a metaDMG-style lca file. Please use an ngsLCA-style lca file."
        )
        sys.exit()
    nodedata, pmds_in_bam = gather_subs_and_kmers(
        args.in_bam, args.in_lca, kn=args.k, upto=args.upto, stranded=args.stranded
    )
    parse_and_write_node_data(
        nodedata, args.out_tsv, args.out_subs, args.stranded, pmds_in_bam
    )
...
def extract(args):
    ...
    if lca_file_type == "metadmg":
        print(
            "Error: It looks like you're trying to run bamdam extract with a metaDMG-style lca file. Please use an ngsLCA-style lca file."
        )
        sys.exit()
    extract_reads(
        args.in_lca,
...
def krona(args):
    make_krona_xml(
        args.in_tsv, args.in_tsv_list, args.out_xml, args.minreads, args.maxdamage
    )
...
def main():
    ...
    parser_shrink = subparsers.add_parser(
        "shrink", help="Filter the BAM and LCA files."
    )
    parser_shrink.add_argument(
        "--in_lca",
        type=str,
        required=True,
        help="Path to the input LCA file (required)",
    )
    parser_shrink.add_argument(
        "--in_bam",
        type=str,
        required=True,
        help="Path to the input (read-sorted) BAM file (required)",
    )
    parser_shrink.add_argument(
        "--out_lca",
        type=str,
        required=True,
        help="Path to the short output LCA file (required)",
    )
    parser_shrink.add_argument(
        "--out_bam",
        type=str,
        required=True,
        help="Path to the short output BAM file (required)",
    )
    parser_shrink.add_argument(
        "--stranded",
        type=str,
    ...
    parser_shrink.add_argument(
        ...
        type=int,
        default=5,
        help="Minimum read count to keep a node (default: 5)",
    )
    parser_shrink.add_argument(
        "--upto",
        type=str,
        default="family",
        help="Keep nodes up to and including this tax threshold (default: family)",
    )
    parser_shrink.add_argument(
        "--minsim",
        type=float,
        default=0.9,
        help="Minimum similarity to reference to keep an alignment (default: 0.9)",
    )
    parser_shrink.add_argument(
        "--exclude_keywords",
        type=str,
        nargs="+",
        default=[],
        help="Keyword(s) to exclude when filtering (default: none)",
    )
    parser_shrink.add_argument(
        "--exclude_keyword_file",
        type=str,
        default=None,
        help="File of keywords to exclude when filtering, one per line (default: none)",
    )
    parser_shrink.add_argument(
        "--annotate_pmd",
        action="store_true",
        help="Annotate output bam file with PMD tags  (default: not set)",
    )
    parser_shrink.set_defaults(func=shrink)

    # Compute
    parser_compute = subparsers.add_parser(
        "compute", help="Compute tsv and subs files."
    )
    parser_compute.add_argument(
        "--in_bam", type=str, required=True, help="Path to the BAM file (required)"
    )
    parser_compute.add_argument(
        "--in_lca", type=str, required=True, help="Path to the LCA file (required)"
    )
    parser_compute.add_argument(
        "--out_tsv",
        type=str,
    ...
    parser_compute.add_argument(
        ...
        type=str,
        default="family",
        help="Keep nodes up to and including this tax threshold; use root to disable (default: family)",
    )
    ...
    parser_extract = subparsers.add_parser(
        "extract",
        help="Extract alignments of reads containing a keyword in an associated lca file.",
    )
    ...
    parser_extract.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the filtered BAM file (required)",
    )
    parser_extract.add_argument(
        "--keyword",
        type=str,
        required=True,
        help="Keyword or phrase to filter for, e.g. a taxonomic node ID (required)",
    )
    parser_extract.add_argument(
        "--subset_header",
        action="store_true",
        help="Subset the header to only relevant references (default: not set)",
    )
    parser_extract.add_argument(
        "--only_top_ref",
        action="store_true",
        help="Only keep alignments to the most-hit reference (default: not set)",
    )
    parser_extract.set_defaults(func=extract)

    # Plot damage
    parser_plotdamage = subparsers.add_parser(
        "plotdamage",
        help="Produces a postmortem damage plot for a specified taxonomic node using the subs file.",
    )
    group_input_plotdamage = parser_plotdamage.add_mutually_exclusive_group(
        required=True
    )
    group_input_plotdamage.add_argument(
        "--in_subs", nargs="+", help="Input subs file(s)"
    )
    group_input_plotdamage.add_argument(
        "--in_subs_list",
        help="Path to a text file contaning input subs files, one per line",
    )
    parser_plotdamage.add_argument(
        "--tax", type=str, required=True, help="Taxonomic node ID (required)"
    )
    parser_plotdamage.add_argument(
        "--outplot",
        type=str,
        default="damage_plot.png",
        help="Filename for the output plot, ending in .png or .pdf (default: damage_plot.png)",
    )
    parser_plotdamage.add_argument(
        "--ymax", type=str, default="0", help="Maximum for y axis (optional)"
    )
    parser_plotdamage.set_defaults(func=plotdamage)

    # Plot bam info
    parser_plotbaminfo = subparsers.add_parser(
        "plotbaminfo",
        help="Produces a mismatch and read length distribution plot for an input bam.",
    )
    group_input_plotbaminfo = parser_plotbaminfo.add_mutually_exclusive_group(
        required=True
    )
    group_input_plotbaminfo.add_argument(
        "--in_bam", nargs="+", help="Input bam file(s)"
    )
    group_input_plotbaminfo.add_argument(
        "--in_bam_list", help="Path to a text file containing input bams, one per line"
    )
    parser_plotbaminfo.add_argument(
        "--outplot",
        type=str,
        default="baminfo_plot.png",
        help="Filename for the output plot, ending in .png or .pdf (default: baminfo_plot.png)",
    )
    ...
    parser_combine = subparsers.add_parser(
        "combine",
        help="Combine multiple bamdam tsv files to generate a multi-sample tsv.",
    )
    group_input_combine = parser_combine.add_mutually_exclusive_group(required=True)
    group_input_combine.add_argument(
        "--in_tsv", nargs="+", help="List of input tsv file(s)"
    )
    group_input_combine.add_argument(
        "--in_tsv_list",
        help="Path to a text file containing paths to input tsv files, one per line",
    )
    parser_combine.add_argument(
        "--out_tsv",
        type=str,
        default="combined.tsv",
        help="Path to output tsv file name (default: combined.tsv)",
    )
    parser_combine.add_argument(
        "--minreads",
        type=float,
        default=50,
        help="Minimum reads across samples to include taxa (default: 50)",
    )
    parser_combine.add_argument(
        "--include",
        nargs="*",
    ...
    group_input_krona.add_argument(
        "--in_tsv", type=str, nargs="+", help="Path to tsv file(s)"
    )
    group_input_krona.add_argument(
        "--in_tsv_list",
        help="Path to a text file containing paths to input tsv files, one per line",
    )
    parser_krona.add_argument(
        "--out_xml",
        type=str,
        default="out.xml",
        help="Path to output xml file name (default: out.xml)",
    )
    parser_krona.add_argument(
        "--minreads",
        type=int,
        default=100,
        help="Minimum reads across samples to include taxa (default: 100)",
    )
    parser_krona.add_argument(
        "--maxdamage",
        type=float,
        default=None,
        help="Force a maximum value for the 5' C-to-T damage color scale. If not provided, the maximum value is determined from the data, with a minimum threshold of 0.3. (not recommended by default)",
    )
    ...
    if hasattr(args, "stranded") and args.stranded not in ["ss", "ds"]:
        parser.error(
            f"Invalid value for stranded: {args.stranded}. Valid values are ss or ds."
        )
    ...
    if hasattr(args, "k") and (
        not isinstance(args.k, int) or not isinstance(args.k, int) or args.k > 50
    ):
        parser.error(
            f"Invalid integer value for k : {args.k} (max 49, and that is much higher than recommended in any case)"
        )
    if hasattr(args, "upto") and not re.match("^[a-z]+$", args.upto):
        parser.error(
            f"Invalid value for upto: {args.upto}. Must be a string of only lowercase letters."
        )
    ...
    if hasattr(args, "upto") and args.upto != args.upto.lower():
        parser.warning(
            f"Warning: {args.upto} as provided is not in lowercase, but it should be. Converting to lowercase and moving on."
        )
    ...
    if hasattr(args, "in_bam") and args.command != "plotbaminfo":
        ...
        if sortorder != "queryname":
            print(
                "Error: Your bam file does not appear to be read-sorted. Please try again with it once it has been read-sorted (samtools sort -n), which should be the same order as your lca file. \n"
            )
            exit(-1)

    if (
        hasattr(args, "in_tsv")
        and hasattr(args, "in_tsv_list")
        and args.in_tsv is not None
        and args.in_tsv_list is not None
    ):
        raise ValueError(
            "You cannot specify both --in_tsv and --in_tsv_list at the same time. Use one or the other."
        )
    if (
        hasattr(args, "in_subs")
        and hasattr(args, "in_subs_list")
        and args.in_subs is not None
        and args.in_subs_list is not None
    ):
        raise ValueError(
            "You cannot specify both --in_subs and --in_subs_list at the same time. Use one or the other."
        )
    if (
        hasattr(args, "in_bams")
        and hasattr(args, "in_bam_list")
        and args.in_bam is not None
        and args.in_bam_list is not None
    ):
        raise ValueError(
            "You cannot specify both --in_bam and --in_bam_list at the same time. Use one or the other."
        )
    ...
    if hasattr(args, "include"):
        invalid_metrics = set(args.include) - {
            "damage",
            "duplicity",
            "dust",
            "taxpath",
        ...
        if invalid_metrics:
            raise ValueError(
                f"Invalid metrics in --include: {', '.join(invalid_metrics)}. Allowed values are: 'damage', 'duplicity', 'dust', 'taxpath', 'all', 'none', or combinations of the first four."
            )
    ...
    if not tqdm_imported:
        print(
            "The library tqdm is not available, so progress bars will not be shown. This will not impact performance."
        )
